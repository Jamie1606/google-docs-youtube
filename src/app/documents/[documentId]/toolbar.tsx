...
const HeadingLevelButton = () => {
  ...
  const setNewFontSize = (value: number) => {
    console.log(value);
    if (value === 0) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="p-1 flex flex-col justify-center gap-y-1">
        {headings.map(({ label, value, fontSize }, index) => (
          <DropdownMenuItem key={index} className={cn("flex items-center text-sm gap-0 px-2 py-0 w-full cursor-pointer rounded-sm hover:bg-neutral-300/80", (value === 0 && !editor?.isActive("heading")) || (editor?.isActive("heading", { level: value }) && "bg-neutral-300/80"))} onClick={() => setNewFontSize(value)}>
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="p-1 flex flex-col gap-y-1">
        {fonts.map(({ label, value }, index) => (
          <DropdownMenuItem key={index} className={cn("flex items-center text-sm cursor-pointer gap-x-2 px-2 py-1 rounded-sm hover:bg-neutral-300/80", editor?.getAttributes("textStyle").fontFamily === value && "bg-neutral-300/80")} onClick={() => setNewFont(value)}>
...
const ToolbarButton = ({ onClick, isActive, icon: Icon }: ToolbarButtonProps) => {
  return (
    <button onClick={onClick} className={cn("text-sm h-7 min-w-7 flex items-center justify-center rounded-sm hover:bg-neutral-300/80", isActive && "bg-neutral-300/80")}>
